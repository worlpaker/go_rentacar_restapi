...
func fakeNewServer(m *mongo.Client) *http.ServeMux {
	...
	// setup all routers once
	r.HandleFunc("/api/locations/show", Handle(s.ShowLocations).Methods("GET"))
	r.HandleFunc("/api/cars/showreservedcars", Handle(s.ShowReservedCars).Methods("GET"))
	r.HandleFunc("/api/cars/reserve", Handle(s.ReserveCar).Methods("POST"))
	r.HandleFunc("/api/cars/available", Handle(s.AvailableCars).Methods("GET"))
...
func TestAvailableCars(t *testing.T) {
	...
	mt.Run("success", func(mt *mtest.T) {
		router := fakeNewServer(mt.Client)
